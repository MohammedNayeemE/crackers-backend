...
const getOrderStatus = async (req : Request , res : Response) =>{
    ...
    try{
        let data;
        if(ck){  data = await client.query(Admin.get_complete);  }
...
const update_items = async (req : Request , res : Response) =>{
    ...
    try{
        ...
        const {item_id , item_name , item_price , item_image , item_stock , item_description} = req.body;
        const values = [ 
            item_name || null ,
...
const add_item = async (req : Request , res : Response) =>{
    const client = await pool.connect();
    try{
        const {item_name , item_price , item_image , item_stock , item_description} = req.body;
        const {rows , rowCount} = await client.query(Admin.add_item , [item_name , item_price , item_image , item_stock , item_description]);
...
const add_category = async (req : Request , res : Response) =>{
    ...
    const {name} = req.body;
    try{
        const {rows , rowCount} = await client.query(Admin.add_category);
        if(rowCount === 0) return res.status(404).json({statusCode : 404 , msg : "you are a fucking dumbass"});
...
const MODULE = {adminSignIn , adminSignUp , getOrderStatus , getItems , get_categories ,update_items , add_item , add_category};
